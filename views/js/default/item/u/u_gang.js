$(function () {
  for (var i = 0; i < item_performance_unit.length; i++) {
    jQuery("<option/>", {
      value: item_performance_unit[i],
      html: item_performance_unit[i],
    }).appendTo("#item_unit");
    jQuery("<option/>", {
      value: item_performance_unit[i],
      html: item_performance_unit[i],
...
async function initItem(id) {
  ...
  $("#upd_item_ot").val(aux[0]);
  $("#upd_item_o").val(aux[1]);
  $("#upd_item_a").val(aux[2]);
}
